                ...
                if is_valid == true then
                    ...
                    if srcTo ~= nil then
                        ...
                        if checkIfInCall(srcTo) then
                            TriggerClientEvent("pNotify:SendNotification", source, {text = "<h3>📱 Telefonsvarer</h3> <br> <p>Nummeret er optaget</P>", type = "error", timeout = 9000, layout = "centerLeft"})
                        else
...
AddEventHandler('gcPhone:allUpdate', function(user_id, source)
    ...
    vRP.getUserIdentity({user_id, function(identity)
        if identity then
            ...
            if #rows > 0 then
                TriggerClientEvent("gcPhone:RefreshTwitter",lsource,json.encode(rows))
            end
			end)
...
RegisterServerEvent('gcPhone:updatekaz')
AddEventHandler('gcPhone:updatekaz', function()
    local kazsource = source
	local user_id = vRP.getUserId({source})
	TriggerEvent("gcPhone:allUpdate", user_id,source)
	end)
